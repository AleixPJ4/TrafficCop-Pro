...
function improveSpanishPoliceText(text, fieldName){
  ...
  if(fieldName==="observaciones" && !/^(Se observa|Se hace constar|Asimismo)/i.test(value)){
    value="Se hace constar que "+value.charAt(0).toLowerCase()+value.slice(1);
  }
...
async function improveCurrentDocument(){
  ...
  status.textContent=changed
    ? `Texto corregido y mejorado en ${changed} apartado${changed===1?"":"s"}. Revisa el resultado antes de utilizarlo.`
