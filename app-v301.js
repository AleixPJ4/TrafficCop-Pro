...
function setup(){
  ...
  if(installButton){
    installButton.addEventListener("click",()=>{
      alert("iPhone: Safari → Compartir → Añadir a la pantalla de inicio.\nAndroid: Chrome → menú → Instalar aplicación.");
    });
  }
...
function setupPro(){
  ...
  if(searchInput){
    ...
    searchInput.addEventListener("input",()=>{
      ...
      timer=setTimeout(()=>{
        ...
        if(value.length>=3) saveRecent(value);
      },900);
    });
  }

  document.querySelectorAll("[data-query]").forEach(button=>{
    button.addEventListener("click",()=>saveRecent(button.dataset.query));
  });

  const clearRecent=$("#clearRecentButton");
  if(clearRecent){
    clearRecent.onclick=()=>{
  ...
  const serviceButton=$("#serviceModeButton");
  const serviceStatus=$("#serviceStatus");
  if(serviceButton && serviceStatus){
    const apply=active=>{
      serviceButton.classList.toggle("active",active);
      serviceStatus.textContent=active ? "Modo servicio activo" : "Modo consulta";
      localStorage.setItem("tcp-v30-service",active?"on":"off");
    };
    apply(localStorage.getItem("tcp-v30-service")==="on");
    serviceButton.onclick=()=>{
      const active=!serviceButton.classList.contains("active");
      apply(active);
      showToast(active ? "Modo servicio activado" : "Modo servicio desactivado");
    };
  }

  document.querySelectorAll("[data-pro-nav]").forEach(button=>{
    button.onclick=()=>{
      document.querySelectorAll("[data-pro-nav]").forEach(item=>item.classList.toggle("active",item===button));
      const action=button.dataset.proNav;
      if(action==="home") window.scrollTo({top:0,behavior:"smooth"});
      if(action==="search"){
        $("#catalogSection")?.scrollIntoView({behavior:"smooth"});
        setTimeout(()=>$("#searchInput")?.focus(),400);
      }
      if(action==="documents") openSuiteModule("documents");
      if(action==="more") openDrawer();
    };
  });

  const customize=$("#customizeQuickButton");
  if(customize) customize.onclick=()=>showToast("Personalización próximamente");

  const onboarding=$("#onboardingDialog");
  const finish=$("#finishOnboardingButton");
  if(onboarding && finish){
    if(localStorage.getItem("tcp-v30-onboarded")!=="yes"){
      setTimeout(()=>onboarding.showModal(),350);
    }
    finish.onclick=()=>{
      localStorage.setItem("tcp-v30-onboarded","yes");
      onboarding.close();
    };
  }

  document.querySelectorAll(".drawer-footer small").forEach(element=>{
    element.textContent="TrafficCop Pro v3.0.1";
  });
}
...
function safeInitialize(){
  try{
    ...
    console.error("TrafficCop Pro 3.0.1:",error);
    const results=$("#results");
    if(results){
      results.innerHTML='<div class="error-panel"><strong>No se ha podido iniciar una función.</strong><p>Recarga la página y comunica el error indicando la versión 3.0.1.</p></div>';
    }
  }
}
safeInitialize();
