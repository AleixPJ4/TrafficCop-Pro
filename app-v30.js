...
const PRO_VERSION="3.0",RECENT_KEY="tcp-v30-recents";let toastTimer=null;
function showToast(m){const t=$("#toast");if(!t)return;t.textContent=m;t.classList.add("show");clearTimeout(toastTimer);toastTimer=setTimeout(()=>t.classList.remove("show"),2200)}
function getRecents(){try{return JSON.parse(localStorage.getItem(RECENT_KEY)||"[]")}catch{return[]}}
function saveRecent(q){q=String(q||"").trim();if(q.length<2)return;const l=[q,...getRecents().filter(x=>normalize(x)!==normalize(q))].slice(0,10);localStorage.setItem(RECENT_KEY,JSON.stringify(l));renderRecents()}
function renderRecents(){const b=$("#recentSearches");if(!b)return;const l=getRecents();b.innerHTML="";if(!l.length){b.innerHTML='<span class="recent-empty">Aún no hay consultas guardadas.</span>';return}l.forEach(q=>{const x=document.createElement("button");x.className="recent-chip";x.textContent=q;x.onclick=()=>{$("#searchInput").value=q;state.limit=50;render();$("#catalogSection").scrollIntoView({behavior:"smooth"})};b.appendChild(x)})}
function setupPro(){let timer;$("#searchInput").addEventListener("input",()=>{clearTimeout(timer);timer=setTimeout(()=>{const v=$("#searchInput").value.trim();if(v.length>=3)saveRecent(v)},900)});document.querySelectorAll("[data-query]").forEach(b=>b.addEventListener("click",()=>saveRecent(b.dataset.query)));$("#clearRecentButton").onclick=()=>{localStorage.removeItem(RECENT_KEY);renderRecents();showToast("Historial borrado")};renderRecents();const sb=$("#serviceModeButton"),ss=$("#serviceStatus");const apply=a=>{sb.classList.toggle("active",a);ss.textContent=a?"Modo servicio activo":"Modo consulta";localStorage.setItem("tcp-v30-service",a?"on":"off")};apply(localStorage.getItem("tcp-v30-service")==="on");sb.onclick=()=>{const a=!sb.classList.contains("active");apply(a);showToast(a?"Modo servicio activado":"Modo servicio desactivado")};document.querySelectorAll("[data-pro-nav]").forEach(b=>b.onclick=()=>{document.querySelectorAll("[data-pro-nav]").forEach(x=>x.classList.toggle("active",x===b));const a=b.dataset.proNav;if(a==="home")window.scrollTo({top:0,behavior:"smooth"});if(a==="search"){$("#catalogSection").scrollIntoView({behavior:"smooth"});setTimeout(()=>$("#searchInput").focus(),400)}if(a==="documents")openSuiteModule("documents");if(a==="more")openDrawer()});$("#customizeQuickButton").onclick=()=>showToast("Personalización próximamente");const d=$("#onboardingDialog");if(localStorage.getItem("tcp-v30-onboarded")!=="yes")setTimeout(()=>d.showModal(),350);$("#finishOnboardingButton").onclick=()=>{localStorage.setItem("tcp-v30-onboarded","yes");d.close()};document.querySelectorAll(".drawer-footer small").forEach(e=>e.textContent="TrafficCop Pro v3.0")}
function safeInitialize(){try{setup();setupVoiceAndAI();setupSideMenu();setupSuite();setupDocumentBuilder();setupDocumentAI();setupPro()}catch(e){console.error(e);const r=$("#results");if(r)r.innerHTML='<div class="error-panel"><strong>No se ha podido iniciar una función.</strong><p>Recarga la página y comunica el error indicando la versión 3.0.</p></div>'}}
safeInitialize();
