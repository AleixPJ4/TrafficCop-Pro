...
const ticketFieldIds=[
  "ticketDate","ticketTime","ticketPlace","ticketTown","ticketPlate","ticketCountry",
  "ticketBrand","ticketModel","ticketPersonId","ticketLicenseClass","ticketLicenseDate",
  "ticketSurname","ticketName","ticketBirthDate","ticketNationality","ticketAddress",
  "ticketOfficerTip","ticketNotifierTip","ticketObservations"
];
...
const ruleDisplay=norma=>{
  const direct=["RGC","RGV","RGCond","AOV"];
  return direct.includes(norma) ? `☒ ${norma}` : `Altres: ${norma}`;
};
function updateTicketSheet(){
  if(!currentTicketItem)return;
  const x=currentTicketItem;
  const date=formatDateEs(val("ticketDate"));
  const time=val("ticketTime")||"—";
  $("#sheetDateTime").textContent=`${date} · ${time}`;
  $("#sheetNorm").textContent=ruleDisplay(x.norma);
  $("#sheetArticle").textContent=x.article||"—";
  $("#sheetAmount").textContent=euro(x.quantia);
  $("#sheetPoints").textContent=String(x.punts??"—");
  $("#sheetPlace").textContent=val("ticketPlace")||"—";
  $("#sheetTown").textContent=val("ticketTown")||"—";
  $("#sheetVehicle").textContent=`Matrícula: ${val("ticketPlate")||"—"} · País: ${val("ticketCountry")||"—"} · Marca: ${val("ticketBrand")||"—"} · Model: ${val("ticketModel")||"—"}`;
  $("#sheetFact").textContent=x.concepte||"—";
  $("#sheetPerson").textContent=`Document: ${val("ticketPersonId")||"—"} · Classe: ${val("ticketLicenseClass")||"—"} · Expedició: ${formatDateEs(val("ticketLicenseDate"))} · Cognoms i nom: ${[val("ticketSurname"),val("ticketName")].filter(Boolean).join(", ")||"—"} · Naixement: ${formatDateEs(val("ticketBirthDate"))} · Nacionalitat: ${val("ticketNationality")||"—"} · Domicili: ${val("ticketAddress")||"—"}`;
  $("#sheetDiscount").textContent=euro(x.descompte);
  $("#sheetOfficer").textContent=`TIP ${val("ticketOfficerTip")||"—"}`;
  $("#sheetNotifier").textContent=`TIP ${val("ticketNotifierTip")||"—"}`;
  $("#sheetObservations").textContent=val("ticketObservations")||"Camp a emplenar per l’agent segons els fets comprovats.";
  $("#sheetBottomAmount").textContent=`Import reduït: ${euro(x.descompte)}`;
}
function openTicket(x){
  currentTicketItem=x;
  $("#ticketTitle").textContent=x.concepte;
  $("#ticketNorm").textContent=x.norma;
  $("#ticketArticle").textContent=x.article;
  $("#ticketAmount").textContent=euro(x.quantia);
  $("#ticketDiscount").textContent=euro(x.descompte);
  $("#ticketPoints").textContent=String(x.punts??"—");
  $("#ticketNature").textContent=nature(x.naturalesa);
  $("#ticketOfficialConcept").textContent=x.concepte;
  $("#ticketSource").textContent=`Font: ${x.font||"Catàleg SCT"} · pàgina ${x.pagina||"—"}`;
  updateTicketSheet();
  openDialog("#ticketDialog");
}
function setupTicket(){
  $("#ticketCloseButton").onclick=()=>closeDialog("#ticketDialog");
  $$("[data-ticket-tab]").forEach(button=>button.onclick=()=>{
    $$("[data-ticket-tab]").forEach(item=>item.classList.toggle("active",item===button));
    ["summary","bulletin","guide"].forEach(key=>{
      const panel=document.getElementById(`ticket${key.charAt(0).toUpperCase()+key.slice(1)}Panel`);
      panel?.classList.toggle("active",button.dataset.ticketTab===key);
    });
  });
  ticketFieldIds.forEach(id=>document.getElementById(id)?.addEventListener("input",updateTicketSheet));
  $("#ticketFillDemoButton").onclick=()=>{
    const now=new Date();
    $("#ticketDate").value=now.toISOString().slice(0,10);
    $("#ticketTime").value=now.toTimeString().slice(0,5);
    $("#ticketPlace").value="Carrer d’exemple, 1";
    $("#ticketTown").value="Població";
    $("#ticketCountry").value="E";
    $("#ticketOfficerTip").value="0000";
    $("#ticketNotifierTip").value="0000";
    updateTicketSheet();
    toast("Exemple variable emplenat");
  };
  $("#ticketClearButton").onclick=()=>{
    ticketFieldIds.forEach(id=>{
      const field=document.getElementById(id);
      if(field)field.value=id==="ticketCountry"?"E":"";
    });
    updateTicketSheet();
  };
...
function init(){setupCatalog();setupDrawer();setupAI();setupDocuments();setupSuite();setupNav();setupTicket();console.log('TrafficCop Pro 3.1.0 iniciado correctamente')}
