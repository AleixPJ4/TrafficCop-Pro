...
function awOperationalViews(){
  ...
  document.querySelectorAll("[data-aw-tab]").forEach(button=>{
    button.onclick=()=>{
      ...
      if(key==="diligences") awShowDiligences();
...
function init(){setupCatalog();setupDrawer();setupDrawerBehaviourFix();setupDesktopMenuFix();setupAI();setupDocuments();setupSuite();setupNav();setupTicket();setupAtestados();setupAtestadosManager();setupAtestadosPolish();setupFullscreenAndButtons();setupDesktopRail();console.log('TrafficCop Pro 3.4.2 iniciado correctamente')}
